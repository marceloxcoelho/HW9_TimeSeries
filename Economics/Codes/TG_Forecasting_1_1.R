...
objList=test;
getSymbols(objList,src="yahoo",from= "2015-02-07"); #downloads data and saves them 
...
decomp=decompose(GGL,type="additive")
autoplot(decomp)

detrend=GGL-decomp$trend
autoplot(detrend)

deseason=GGL-decomp$seasonal
autoplot(deseason)
...
df1$MA_7_VALUE=ma(df1$ADJUSTED,7)
...
TS_All=c(TS_Train,TS_Test) ###STOP
